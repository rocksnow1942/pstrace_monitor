...
def main(file):
    ...
    addToFile = output(outdir/(f.name + '.txt'))
    ...
    addToFile("原始数据图:")
    addToFile(json.dumps([list(np.linspace(0,30,90)),list(c)]))
    ...
    addToFile("图1:")
    addToFile(json.dumps([list(np.linspace(0,30,90)),list(smoothed_X[0][1])]))
    ...
    smoothed_X = smoothT.transform(X)
    addToFile("图3:")
    addToFile(json.dumps([list(smoothed_X[0][0]),list(smoothed_X[0][1])]))
    ...
    addToFile(json.dumps([list(xvals),list(hyperline(xvals))]))
...
import glob
files = glob.glob("/Users/hui/Work/Covid_Reader/TWXY/20220204dataSentToTWXY/rawData/*.json")
for file in files:
    main(file)